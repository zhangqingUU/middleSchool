...
function saveInfo() {
    ...
    var addressOld = $(".display-address").css("display");
    var detail_address = "";
    if (addressOld == "none") {
        //地址使用-拼接
        detail_address = $(".stu-address1 option:selected").text() + "-" + $(".stu-address2 option:selected").text() + "-" + $(".stu-address3 option:selected").text() + "-" + $(".stu-address4").val();
    }else{
        detail_address =$(".display-address").val();
    }
    ...
    console.log(json);
    $.post("/student/updStu",json,function(result){
        console.log(result);
        if(result==true){
            alert("修改成功！");
        }else{
